...
namespace RockWeb.Plugins.church_vrl.FlexiContentComponents
{
    /// <summary>
    ///
    /// </summary>
    [DisplayName( "Flexi Content Components - VRL" )]
    [Category( "CMS" )]
    [Description( "A content component that lets you choose which content channel type it uses and more." )]
    ...
    [Rock.SystemGuid.BlockTypeGuid( "3D096264-0FDE-4B0B-B1A0-2C4DDA1EE4DA" )]
    public partial class FlexiContentComponents : RockBlockCustomSettings
    {
...
public override List<Control> GetAdministrateControls( bool canConfig, bool canEdit )
        {
            ...
            // Find the standard block properties gear, but DO NOT remove it.
            // We need it to stay so users can set the ContentChannelType attribute.
            ...
            if ( canConfig )
            {
                ...
                lbConfigure.ToolTip = "Component Configuration";
                lbConfigure.Click += lbConfigure_Click;
                
                HtmlGenericControl iConfigure = new HtmlGenericControl( "i" );
                // Use a different icon (sliders) to visually distinguish it from the standard gear
                iConfigure.Attributes.Add( "class", "fa fa-sliders" ); 
                ...
                {
                    // Insert the custom gear right next to the standard gear
        ...
        {
            // Defensive Check: Ensure the base block properties have been set first
            if ( this.GetContentChannelTypeId() == 0 )
            {
                nbContentError.Text = "Please set the Content Channel Type in the block properties (the gear icon) before saving the component configuration.";
                nbContentError.NotificationBoxType = NotificationBoxType.Danger;
                nbContentError.Visible = true;
                
                // Keep the modal open so they can see the error
                mdContentComponentConfig.Show(); 
                return;
            }
        ...
        private void EditContentChannelItem( int? contentChannelItemId )
        {
            ...
            if ( contentChannelItem == null )
            {
                ...
                // Initialize default time to now for new records to prevent empty Start Date values
                contentChannelItem.StartDateTime = RockDateTime.Now; 
            }
            ...
            // Retrieve the ContentChannelType to respect its Date and Time settings
            var contentChannelType = ContentChannelTypeCache.Get( contentChannelItem.ContentChannelTypeId );
            bool includeTime = contentChannelType != null && contentChannelType.IncludeTime;
            bool isDateRange = contentChannelType != null && contentChannelType.DateRangeType == Rock.Model.ContentChannelDateType.DateRange;

            // Configure Date View Layout
            pnlExpireDate.Visible = isDateRange;
            pnlStartDate.CssClass = isDateRange ? "col-md-6" : "col-md-12";

            dtpStartDateTime.Visible = includeTime;
            dpStartDate.Visible = !includeTime;

            dtpExpireDateTime.Visible = includeTime;
            dpExpireDate.Visible = !includeTime;

            if ( includeTime )
            {
                dtpStartDateTime.SelectedDateTime = contentChannelItem.StartDateTime;
                dtpExpireDateTime.SelectedDateTime = contentChannelItem.ExpireDateTime;
            }
            else
            {
                dpStartDate.SelectedDate = contentChannelItem.StartDateTime.Date;
                dpExpireDate.SelectedDate = contentChannelItem.ExpireDateTime.HasValue ? contentChannelItem.ExpireDateTime.Value.Date : (DateTime?)null;
            }
        ...
        private void SaveContentChannelItem()
        {
            ...
            // Retrieve the ContentChannelType to respect its Date and Time settings during save
            var contentChannelType = ContentChannelTypeCache.Get( contentChannelItem.ContentChannelTypeId );
            bool includeTime = contentChannelType != null && contentChannelType.IncludeTime;
            bool isDateRange = contentChannelType != null && contentChannelType.DateRangeType == Rock.Model.ContentChannelDateType.DateRange;

            // Map DateTimes from controls
            if ( includeTime )
            {
                contentChannelItem.StartDateTime = dtpStartDateTime.SelectedDateTime ?? RockDateTime.Now;
                contentChannelItem.ExpireDateTime = isDateRange ? dtpExpireDateTime.SelectedDateTime : (DateTime?)null;
            }
            else
            {
                contentChannelItem.StartDateTime = dpStartDate.SelectedDate ?? RockDateTime.Now.Date;
                contentChannelItem.ExpireDateTime = isDateRange ? dpExpireDate.SelectedDate : (DateTime?)null;
            }
